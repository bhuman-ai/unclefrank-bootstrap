// FRANK'S TMUX-BASED CLAUDE EXECUTOR
// Uses tmux to run Claude Code in persistent sessions - ACTUALLY WORKS!
...
async function sendToClaudeTmux(tmuxSession, message) {
    try {
        ...
        const { stdout } = await execAsync(`tmux capture-pane -t ${tmuxSession} -p`);
...
app.listen(PORT, () => {
    console.log(`Uncle Frank's TMUX-BASED Claude Executor running on port ${PORT}`);
    console.log(`GitHub repo: ${GITHUB_REPO}`);
    console.log(`Using tmux for persistent Claude sessions`);
});